...
public class UpdateAction extends GenericAction {
    ...
    private static Logger log = Logger.getLogger(UpdateAction.class);
        ...
        try {
            ...
            if (sessionBean != null) {
                ...
                if (validateProduct(request)) {
                    ...
                    int addResult = ProductInfoHelper.getInstance().update(product);
                    if (addResult == 1) {
                        result = mapping.findForward("success");
                    } else {
                        result = mapping.findForward("updateError");
                    }
